...
export const stripeWebhook = async (req, res) => {
  ...
  try {
    ...
    const header = loadStripe.webhooks.generateTestHeaderString({
      payload: payloadString,
      secret,
    });

    event = paypal.webhooks.constructEvent(payloadString, header, secret);
  } catch (error) {
...
//     if (!order?.result?.id) {
//       return res.status(400).json({ success: false, message: "Error creating PayPal order" });
//     }
...
//   } catch (error) {
//     console.error(error);
//     res.status(500).json({ message: "Something went wrong while creating PayPal session" });
//   }
// };
// export const getAllPurchasedCourse = async (_, res) => {
//       try {
