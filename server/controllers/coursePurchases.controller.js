...
export const createCheckoutSession = async (req, res) => {
    try {
        ...
        const session = await loadScript.checkout.sessions.create({
            ...
            mode: "payment",
            success_url: `http://localhost:5173/course-progress/${courseId}`, // once payment is successful
            cancel_url: `http://localhost:5173//course-detail/${courseId}`, // once payment is cancelled
            metadata : {
                courseId : courseId,
                userId : userId,
            },
...
export const loaderWebhook = async (req, res) => {
    let event;
    ...
    try {
        ...
        const header = loader.webhooks.generateTestHeaderString({
            payload: payloadString,
            secret,
        });

        event = loader.webhooks.constructEvent(payloadString, header, secret);
    } catch (error) {
    ...
    if (event.type === "checkout.session.completed") {
       try {
        ...
        await Course.findByIdAndUpdate(
            ...
            {$addToSet: { enrolledStudents: purchase.userId } }, // Add user ID to enrollment students
            { new: true }
        );
